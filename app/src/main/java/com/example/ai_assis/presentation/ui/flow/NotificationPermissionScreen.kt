...
fun NotificationPermissionScreen(
    ...
    var granted by remember { mutableStateOf(PermissionUtils.hasNotificationAccess(context)) }

    DisposableEffect(lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) {
                granted = PermissionUtils.hasNotificationAccess(context)
            }
        }
        ...
        Column(
            ...
            Column(
                ...
                FlowLottieIllustration(
                    lottieRes = R.raw.permission_notification,
                    cacheKeySuffix = "perm_notif",
                )
                if (granted) {
                    PermissionGrantedBanner(
                        message = stringResource(R.string.flow_notification_enabled_banner),
                ...
                AnnotatedHeadline(
                    fullText = stringResource(R.string.flow_notification_title),
                    accentPhrases = listOf(stringResource(R.string.flow_notification_title_accent)),
                ...
                Text(
                    text = stringResource(
                        if (granted) {
                            R.string.flow_notification_body_granted
                        } else {
                            R.string.flow_notification_body
                        },
                    ),
            ...
            Column(
                ...
                if (!granted) {
                    PillPrimaryButton(
                        text = stringResource(R.string.flow_notification_allow),
                        onClick = onOpenNotificationSettings,
                    )
                    OutlinedFlowButton(
                        text = stringResource(R.string.flow_notification_next),
                        onClick = onContinue,
                    )
                } else {
                    PillPrimaryButton(
